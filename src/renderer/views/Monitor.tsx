...
export default function Monitor() {
  ...
  function submit(cmd: string, alter: boolean=false) {
    if (!alter) {
  ...
  const addBook = () => {
    if (!update)
        submit(`INSERT INTO book (name, intro, author, publisher) VALUES ("${name}", "${intro}", ${author}, ${publisher})`, true);
    else {
  ...
  const getBooksWithCmt = () => {
    submit(`SELECT * FROM book WHERE EXISTS (SELECT * FROM comment WHERE comment_to == bid)`)
  }

  const getBooksNoCmt = () => {
    submit(`SELECT * FROM book WHERE NOT EXISTS (SELECT * FROM comment WHERE comment_to == bid)`)
  }

  const getCmtPerReader = () => {
    submit(`SELECT rid, name, COUNT(cid) AS cnt FROM reader, comment WHERE author == rid GROUP BY rid;`)
  }

  const getNumTxtPerReader = () => {
    submit(`SELECT rid, SUM(len) FROM (SELECT rid, LENGTH(content) AS len FROM reader, comment WHERE reader.rid == comment.author) GROUP BY rid`);
  }

  const getBookWithMaxCmt = () => {
    submit('SELECT bid, name, MAX(cmt) FROM (SELECT bid, book.name AS name, COUNT(cid) AS cmt, pid FROM book, publisher, comment WHERE bid == comment_to AND book.publisher == publisher.pid AND publisher.pid == 7 GROUP BY comment_to) GROUP BY pid');
  }

  const getBookWithMinCmt = () => {
    submit('SELECT bid, name, MIN(cmt) FROM (SELECT bid, book.name AS name, COUNT(cid) AS cmt, pid FROM book, publisher, comment WHERE bid == comment_to AND book.publisher == publisher.pid AND publisher.pid == 7 GROUP BY comment_to) GROUP BY pid');
  }

  const getAvgCmtPerReader = () => {
    submit('SELECT AVG(cmt) FROM (SELECT rid, COUNT(cid) AS cmt FROM reader, comment WHERE author == rid GROUP BY author)');
  }

  const getAuthorWith2moreBooks = () => {
    submit('SELECT aid, author.name AS name, COUNT(bid) AS cnt FROM author, book WHERE aid == book.author GROUP BY aid HAVING cnt >= 2');
  }
  ...
  return (
    <Stack direction="column">
      ...
      <Stack direction={'row'}>
         <Button color="white" colorScheme="cyan" onClick={getAuthorWith2moreBooks}>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          {!update ? <ModalHeader>Add a New Book</ModalHeader> : <ModalHeader>Edit Book Info</ModalHeader>}
          <ModalCloseButton />
          <ModalBody>
            {update ? <Text marginLeft="5px" marginBottom="5px">
              Choose Your Book
            </Text> : <></>}
            {update ? <Select
            ...
            {!update ? <Input placeholder="Enter Book Title" onChange={handleBookName} /> : <></>}
            {!update ? <Text marginLeft="5px" marginBottom="5px">
              Author
            </Text> : <></>}
            {!update ? <Select placeholder="Select Author" onChange={handleAuthor}>
              {authors.map((a, i) => (
          ...
          <ModalFooter>
            {!update ? <Button colorScheme="blue" marginRight="10px" onClick={addBook}>
              Add
            </Button> : <Button colorScheme="blue" marginRight="10px" onClick={addBook}>
              Update
            </Button>}
